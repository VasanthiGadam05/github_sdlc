...
| FR-4 | The report writer replaces the complete destination file. |
...
The implementation SHALL not follow directory symlinks during source
discovery, preventing traversal outside the supplied source root. Discovery
continues to include regular `.py` files reached through ordinary directory
entries and does not read source contents.

The report writer SHALL write the complete Markdown document to a temporary
file within `docs/generated/`, flush and close it, and atomically replace
`source-files.md`. A failed write SHALL leave an existing report unchanged
where supported by the filesystem.

As an operational performance constraint, discovery and report generation
SHALL complete for 500 regular Python files in under three seconds on the
supported development environment. Phase 7 SHALL verify this with a bounded
